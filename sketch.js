...
function preload() {
  // only load background + font here:
  bg          = loadImage('bg.png');
...
function drawNames() {
  ...
  // FRONT name: left-aligned, trimmed to box
  textAlign(LEFT, TOP);
  {
    let s = frontName;
    const maxW = (FRONT_NAME_END_X - FRONT_NAME_START_X)*size_ratio;
    while (textWidth(s) > maxW && s.length) s = s.slice(0,-1);
...
function loadRandomPokemon() {
  ...
  const backP  = pokemonList[j];

  // Generate their new HP values
  const nextHPFront = random(0.3, 1);
  const nextHPBack  = random(0.3, 1);

  // And their names
  const nextFrontName = frontP.name;
  const nextBackName  = backP.name;

  // When the front‐sprite image finishes loading, swap it in:
  loadImage(`front/${frontP.file}`, img => {
    frontSprite = img;
    frontName   = nextFrontName;
    hpFront     = nextHPFront;
  });

  // Likewise for the back‐sprite:
  loadImage(`back/${backP.file}`, img => {
    backSprite = img;
    backName   = nextBackName;
    hpBack     = nextHPBack;
  });

  // Reset your timer now (so the next auto‐swap won’t fire immediately):
  lastSwapTime = millis();
}
